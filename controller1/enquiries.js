...
const createenquiries = async (req, res) => {
  try {
    ...
    const { name, email, phone_no, message, enquiry_status } = req.body;

    const data = await connection.query(
      "INSERT INTO enquiries (name, email, phone_no, message, enquiry_status, ip) VALUES (?, ?, ?, ?, ?, ?)",
      [name, email, phone_no, message, enquiry_status || 0, clientIP]
    );
...
const updatebyidenquiries = async (req, res) => {
  try {
    ...
    const data = await connection.query(
      "UPDATE enquiries SET name=?, email=?, phone_no=?, message=?, enquiry_status=?, ip=?, updated_at=NOW() WHERE enquiry_id=?",
      [name, email, phone_no, message, enquiry_status || 0, clientIP, enquiry_id]
    );
