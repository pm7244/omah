...
const ProjectEdit = () => {
  ...
  // const handleSubCategoryChange2= (selectedOptions) => {
  //   const subCatIds = (selectedOptions || []).map((option) => option.value);
  //   console.log("Selected Sub-Category 1:", subCatIds);
  ...
  const FetchCategory9 = async () => {
    fetch(`${import.meta.env.VITE_CMS_URL}api/getall-pcat`)
      ...
      .catch((error) => console.error("Fetch Category 9 Error:", error));
  };

  const FetchCategory7 = async (cat_id) => {
    if (!cat_id) {
      console.warn("FetchCategory7 called with an invalid cat_id:", cat_id);
      return;
    }
    const url = `${
      import.meta.env.VITE_CMS_URL}api/getall-pcat?cat_id=${cat_id}`;
    ...
    try {
      ...
      if (data.status && data.data) {
        ...
        setOptions7(formattedOptions);
      } else {
        console.error("Fetch Category 7 Error: Invalid response format", data);
      }
    } catch (error) {
      console.error("Fetch Category 7 Error:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    if (values.category_id) {
          FetchCategory7(values.category_id);
        }
      }, [values.category_id]);
  ...
  //     setValues((prev) => ({
  //       ...prev,
  //       category_id: selectedValues,  // Update category_id
  //       sub_category_id: "", // Reset sub_category_id when category changes
  //     }));
  
  //     console.log("Updated category_id in state:", selectedValues);
  
  //     if (selectedValues) {
  //       FetchCategory7(selectedValues); // Fetch subcategories
  //     }
  //   } 
  //   else if (id === "sub_category_id") {
  //     const selectedValues = selectedOption.map((op) => op.value).join(",");
  ...
  const handleProjectSubmit = () => {
    ...
    fetch(`${import.meta.env.VITE_CMS_URL}api/updateProject/${id}`, {
      ...
      body: JSON.stringify({
        ...data,
        cat_id: categoryId,
        // cat_id: JSON.stringify(values.category_id),
  ...
  <div className="box-body">
    <Select
      isMulti
      name="category_id"
      value={options4.filter((op) =>
        (values.category_id || "").split(",").includes(op.value.toString())
      )}
  ...
  <div className="box-body">
    ...
    key={values.category_id}
     ...
     onChange={handleSubCategoryChang1}
      options={options4}
    />
  </div>
</div> 
...
<div className="box">
  ...
  <div className="box-body">
    ...
    key={values.category_id}
      isMulti
      name="category_id"
      // value={options7.filter((op) =>
      //   (values.category_id || "").split(",").includes(op.value.toString())
      // )}
      value={options7}
      
     onChange={handleSubCategoryChang2}
      // options={options7}
    />
  </div>
</div> 
                  ...
                  <div className="box-body">
                    <Select
                      ...
                      onChange={(selectedOptions) =>
                        handleSelectChange("category_id", selectedOptions)
                      }
                      options={options4}
                    />
                  </div>
