...
// const [subCategories, setSubCategories] = useState([]);
  ...
  const FetchCategory7 = async (catIds) => {
    if (!catIds) return [];
  
    try {
      const res = await fetch(
        `${import.meta.env.VITE_CMS_URL}api/getall-pcat?cat_id=${encodeURIComponent(catIds)}`
      );
      const data = await res.json();
  
      if (data.data && data.data.length > 0) {
        // Merge new subcategories with existing ones, preventing duplicates
        setSelectedValue7((prev) => {
          const newSubcategories = data.data.filter(
            (newSub) => !prev.some((prevSub) => prevSub.cat_id === newSub.cat_id)
          );
  ...
  const handleChange4 = (selectedOptions) => {
    setSelectedValue4(selectedOptions);
    setSelectedValue7([]); // Reset subcategories selection
  
    if (!selectedOptions || selectedOptions.length === 0) {
      setSelectedValue7([]); // Reset subcategories if no category is selected
      return;
    }
  
    // Extract category IDs from the selected categories
    const catIds = selectedOptions.map((cat) => cat.cat_id).join(",");
    FetchCategory7(catIds); // Fetch subcategories based on selected category IDs
  };
  

  // Handle Subcategory Selection
  const handleChange7 = (selectedOptions) => {
    setSelectedValue7(selectedOptions);
  };
  ...
  const handleChange5 = (selectedOptions) => {
    const selectedIds = selectedOptions ? selectedOptions.map(option => option.sub_cat_id).join(",") : "";
    
    setSelectedValue5(selectedOptions); 
    setSubCat(selectedIds); 
  };
  ...
  console.log("Selected Value 7:", selectedValue7);
                    ...
                    <div className="box">
                      ...
                      <div className="box-body">
                        <AsyncSelect
                          ...
                          options={FetchCategory7} // Use fetched subcategories as options
                          cacheOptions
                          getOptionLabel={(e) => e.cat_title}
                          getOptionValue={(e) => e.cat_id}
                        />
      ...
      <div className="box">
        ...
        <div className="box-body">
          <AsyncSelect
            ...
            getOptionLabel={(e) => e.cat_title}
            getOptionValue={(e) => e.cat_id}
            // isDisabled={!selectedValue4.length} // Disable if no category is selected
          />
        </div>
      </div>
    </div> */}
                ...
                <div className="box">
                  ...
                  <div className="box-body">
                    <AsyncSelect
                      className="blog-tag2 z-40"
                      isMulti
                      onChange={handleChange5} // Updated to use handleChange5
                      value={selectedValue5}
                      loadOptions={FetchCategory5}
                      cacheOptions
                      defaultOptions
