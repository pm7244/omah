...
const ProjectEdit = () => {
    ...
    fetch(`${import.meta.env.VITE_CMS_URL}api/getbyidproject/${id}`)
      ...
      .then((data) => {
        ...
        try {
          ...
          console.log("Data before setting values:", data.data[0]);
  ...
  const handleSubCategoryChang2 = (selectedOptions) => {
    const subCatIds = (selectedOptions || []).map((option) => option.value);
    console.log("Selected Category 2", subCatIds);
    
  
    setValues((prev) => {
      const updatedValues = {
        ...prev,
        category_id: subCatIds.join(","),
      };
  ...
  const handleSubCategoryChang1 = (selectedOptions) => {
    const subCatIds = (selectedOptions || []).map((option) => option.value);
    console.log("Selected Category 1", subCatIds);
  ...
  const handleSubCategoryChang3= (selectedOptions) => {
    const subCatIds = (selectedOptions || []).map((option) => option.value);
    console.log("Selected Category3:", subCatIds);
    
  
    setValues((prev) => {
      const updatedValues = {
        ...prev,
        sub_category_id: subCatIds.join(","),
      };
      console.log("Updated State:", updatedValues); 
      return updatedValues;
    });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch(`${CMS_URL}api/getall-pcat`); 
  ...
  const getSubcategoryOptions = () => {
    return Object.entries(subcategoriesMap)
      .filter(([catId]) => selectedCategories.includes(parseInt(catId)))
      .flatMap(([, subs]) => subs);
  };


  const fetchSubcategoriesFor = async (catId) => {
    try {
      const res = await fetch(`${CMS_URL}api/getall-pcat?cat_id=${catId}`);
      const data = await res.json();
      ...
      console.error(`Error fetching subcategories for cat_id=${catId}:`, err);
    }
  };


  const handleCategoryChange = async (selected) => {
    const newCatIds = selected ? selected.map((opt) => opt.value) : [];

    const removedCatIds = selectedCategories.filter(
      (id) => !newCatIds.includes(id)
    );

    const remainingSubs = selectedSubcategories.filter(
      (sub) => !removedCatIds.includes(sub.parentId)
    );

    const newlyAddedCatIds = newCatIds.filter(
      (id) => !selectedCategories.includes(id)
    );


    await Promise.all(
      newlyAddedCatIds.map((id) =>
        subcategoriesMap[id] ? Promise.resolve() : fetchSubcategoriesFor(id)
      )
    );

    const newSubs = newlyAddedCatIds.flatMap(
      (id) => subcategoriesMap[id] || []
    );

    const mergedSubs = [...remainingSubs, ...newSubs].filter(
      (v, i, a) => a.findIndex((t) => t.value === v.value) === i
    );

    setSelectedCategories(newCatIds);
    setSelectedSubcategories(mergedSubs);
  };

  const handleSubcategoryChange = (selected) => {
    const newSubs = selected || [];

    const stillUsedCatIds = Array.from(
      new Set(newSubs.map((sub) => sub.parentId))
    );

    setSelectedSubcategories(newSubs);
    setSelectedCategories(stillUsedCatIds);
  };
  ...
  useEffect(() => {
    console.log("State updated with sub_category_id:", values.sub_category_id);
  }, [values.sub_category_id,]);

  useEffect(() => {
    console.log("State updated with category_id:", values.category_id)
  }, [values.category_id]);

  const handleSelectChange = (id, selectedOption) => {
    if (id === "category_id") {
      const selectedValues = selectedOption.map((op) => op.value).join(",");

      setValues((prev) => ({
        ...prev,
        cat_id: selectedValues,
      }));

      console.log("Updated cat_id in state:", selectedValues);
  ...
  const handleInputChange = (e) => {
    ...
    setValues((prv) => ({ ...values, [name]: value }));
  };
  ...
  const handleProjectSubmit = () => {
    ...
    let subCategoryId = values.sub_category_id;
    ...
    fetch(`${import.meta.env.VITE_CMS_URL}api/updateProject/${id}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        console.log("API response my:", data);
        if (data.status) {
...
<div className="box">
  <div className="box-header">
    <h5 className="box-title ">Category & Sub-Category</h5>
  </div>
  <div className="box-body">
    <div className="mb-4">
      <label className="form-label mb-5">Category</label>
      <Select
        isMulti
        options={categoryOptions}
        value={categoryOptions.filter((opt) =>
    ...
    <div>
      <label className="form-label mt-4 fw-bold">Sub-Category</label>
      <Select
        isMulti
        options={getSubcategoryOptions()}
        value={selectedSubcategories}
...
{/* 
                ...
                <div className="box">
                  <div className="box-header">
                    <h5 className="box-title">Sub-Category BHK</h5>
                  </div>
                  <div className="box-body">
                    <Select
                      isMulti
                      name="sub_category_id"
                      value={options5.filter((op) =>
                        (values.sub_category_id || "")
                          .split(",")
                          .includes(op.value.toString())
                      )}
                      onChange={handleSubCategoryChang3}
                      options={options5}
                    />
                  </div>
                </div>
              ...
              <div className="col-span-12 xxl:col-span-4">
                ...
                {/* <div className="box">
                    ...
                    onChange={(selectedOptions) => handleSelectChange("sub_category_id", selectedOptions)}
                    options={options5}
                  />
