...
const UpdatorTeam = () => {
  ...
  const fetchData = () => {
    fetch(`${import.meta.env.VITE_CMS_URL}api/getbyidmanagement/${id}`)
      .then((res) => res.json())
      .then((data) => {
        let rawUrl = JSON.parse(data.data[0].image);
        setImage(rawUrl);
        setValues({ ...data.data[0], image: rawUrl });
      });
  };
  ...
  const handleUpdation = () => {
    ...
    fetch(`${import.meta.env.VITE_CMS_URL}api/updatebyidmanagement/${id}`, {
      method: "PUT",
