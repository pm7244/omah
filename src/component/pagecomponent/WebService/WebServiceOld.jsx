...
const WebPreProjects = () => {
  ...
  const fetchHeroData = () => {
    fetch(`${import.meta.env.VITE_CMS_URL}api/getallwebpreprojects`)
      ...
      .then((data) => {
        ...
        const heroImageData = resData.hero_image ? JSON.parse(resData.hero_image) : [];
  ...
  const handleAddContent = () => {
    ...
    const apiUrl = editingContentIndex !== null && contentData[editingContentIndex]?.sc_id
    ...
    const method = editingContentIndex !== null && contentData[editingContentIndex]?.sc_id ? "PUT" : "POST";
  ...
  const handleDeleteContent = (index) => {
    ...
    if (window.confirm("Are you sure you want to delete this content?")) {
      if (content.sc_id) {
        fetch(`${import.meta.env.VITE_CMS_URL}api/deletebyidservicecontent/${content.sc_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((result) => {
  ...
  const handleMoveContent = async (index, direction) => {
    ...
    try {
      ...
      if (item1.sc_id && item2.sc_id) {
        ...
        const promises = [
          fetch(`${import.meta.env.VITE_CMS_URL}api/updatebyidservicecontent/${item1.sc_id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ 
              ...item1, 
              sort_order: (index + 1).toString(), // Convert to string since DB expects varchar
              image: Array.isArray(item1.image) ? item1.image : item1.image ? [item1.image] : [],
              title: item1.title || "",
              des: item1.des || "",
              status: item1.status || 1
            }),
          }),
          fetch(`${import.meta.env.VITE_CMS_URL}api/updatebyidservicecontent/${item2.sc_id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ 
              ...item2, 
              sort_order: (newIndex + 1).toString(), // Convert to string since DB expects varchar
              image: Array.isArray(item2.image) ? item2.image : item2.image ? [item2.image] : [],
              title: item2.title || "",
              des: item2.des || "",
                ...
                <div className="box">
                  ...
                  <div className="box-body">
                    <div className="space-y-2">
                      ...
                      >
                        {heroImage.length > 0 ? "Change Hero Image" : "Upload Hero Image"}
                      </button>
                      {heroImage.length > 0 && (
                          ...
                          <div className="grid grid-cols-12 gap-4">
                            ...
                            <div className="col-span-12">
                              ...
                              <div className="space-y-2">
                                ...
                                >
                                  {contentImage.length > 0 ? "Change Image" : "Select Image"}
                                </button>
                          ...
                          <tr key={item.sc_id || index}>
                            ...
                            <td>
                              {item.image && item.image.length > 0 ? (
                                <img
                                  src={`${import.meta.env.VITE_CMS_URL}api/transform/${item.image[0]}`}
                                  className="h-12 w-16 rounded-sm object-cover border cursor-pointer"
                                  alt="Content"
                                  onClick={() =>
                                    window.open(
                                      `${import.meta.env.VITE_CMS_URL}api/transform/${item.image[0]}`,
                                      "_blank"
                                    )
                                  }
                                />
