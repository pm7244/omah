...
const Home = () => {
  ...
  const [missionSlider, setMissionSlider] = useState([]);
    ...
    fetch(`${import.meta.env.VITE_CMS_URL}api/getallhome`)
      ...
      .then((data) => {
        ...
        if (data.status && data.data && data.data.length > 0) {
          ...
          setVideo(videoData);
          let heroImages = [];
          if (homeData.hero_image) {
            try {
              heroImages = JSON.parse(homeData.hero_image);
            } catch {
              heroImages = [homeData.hero_image];
            }
          }
          setHeroImage(heroImages);

          let missionImages = [];
          if (homeData.mission_slider) {
            try {
              missionImages = JSON.parse(homeData.mission_slider);
            } catch {
              missionImages = [homeData.mission_slider];
            }
          }
          setMissionSlider(missionImages);

          setValues({
            video: homeData.video || "",
            mission_slider: homeData.mission_slider || "",
            mission_title: homeData.mission_title || "",
            mission_des: homeData.mission_des || "",
            hero_image: homeData.hero_image || "",
            heroimage_text: homeData.heroimage_text || "",
  ...
  useEffect(() => {
    if (missionSlider && missionSlider.length > 0) {
      setValues(prev => ({ ...prev, mission_slider: JSON.stringify(missionSlider) }));
    } else {
      setValues(prev => ({ ...prev, mission_slider: "" }));
    }
  }, [missionSlider]);
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      mission_slider: JSON.stringify(missionSlider),

      mission_title: values.mission_title,
      mission_des: values.mission_des,
      heroimage_text: values.heroimage_text,
              ...
              <div className="col-span-12">
                <div className="grid grid-cols-12 gap-x-6 gap-y-5">
                  ...
                  <div className="col-span-12 md:col-span-6">
                    ...
                    <div className="box">
                      <div className="box-body space-y-5">
                        <label className="ti-form-label">Hero Image Text</label>
                        <input
                          type="text"
                          name="heroimage_text"
                          value={values.heroimage_text || ""}
                          onChange={handleInputChange}
                          className="ti-form-input"
                  ...
                  <div className="col-span-12 md:col-span-6">
                    ...
                    <div className="box">
                      <div className="box-body space-y-5">
                        <label className="ti-form-label">Mission Title</label>
                        <input
                          type="text"
                          name="mission_title"
                    ...
                    <div className="box">
                      ...
                      <div className="box-body space-y-4">
                        ...
                        >
                          ...
                          {heroImage.length > 0 ? "Change Image" : "Upload Image"}
                        </button>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {heroImage.map((img, index) => (
                                <div key={index} className="relative w-full">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setHeroImage(prev => prev.filter((_, i) => i !== index))}
                      ...
                      <div className="box-body space-y-4">
                        {video && video.length > 0 ? (
                          <>
                            ...
                            <video controls className="box-img-top h-40 w-full rounded-t-sm">
                              <source
                  ...
                  <div className="col-span-12">
                    <div className="box">
                      <div className="box-header">
                        <h5 className="box-title">Mission Slider Images</h5>
                      </div>
                      <div className="box-body space-y-4">
                        <button
                          type="button"
                        ...
                        >
                          <i className="ti ti-upload mr-2"></i>
                          {missionSlider.length > 0 ? "Change Images" : "Upload Images"}
                        </button>

                        {missionSlider.length > 0 && (
                          <div className="space-y-2">
                            <label className="text-sm font-medium text-gray-700">Preview:</label>
                            <div className="grid grid-cols-3 gap-2">
                              {missionSlider.map((img, index) => (
                                <div key={index} className="relative w-full">
                                  <img
                                    src={`${import.meta.env.VITE_CMS_URL}api/transform/${img}`}
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setMissionSlider(prev => prev.filter((_, i) => i !== index))
                                    }
                                    className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs"
                                  >
                                    ×
