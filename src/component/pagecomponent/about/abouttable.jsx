...
const TestimonialTable = () => {
  ...
  const fetchData = () => {
    fetch(`${import.meta.env.VITE_CMS_URL}api//getall-about`)
      .then((res) => res.json())
  ...
  const handleDeleteClick = (id) => {
    ...
    if (confirm) {
      fetch(`${import.meta.env.VITE_CMS_URL}api/deletebyid-about/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then(() => {
...
const ReadOnlyRows = ({ contact, handleDeleteClick, setContacts }) => {
  return (
    <tr>
      <td className="text-center">{contact.a_id }</td>
      <td>{contact.b_title}</td>
      <td className="p-2 w-1/2">{contact.b_des.slice(0, 80) + "..."}</td>
      <td className=" grid grid-cols-1 text-end font-medium">
        <div className="flex justify-center">
          <div className="hs-tooltip ti-main-tooltip">
            <Link
              to={`/cms/general/about/editabout/${contact.a_id }`}
          ...
          <div className="hs-tooltip ti-main-tooltip">
            <button
              type="button"
              onClick={() => handleDeleteClick(contact.a_id )}
